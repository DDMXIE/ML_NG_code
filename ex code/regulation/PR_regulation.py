...
def gradientDescent(X1, X2, X3, X4, y, theta_0, theta_1, theta_2, theta_3, theta_4 ,alpha, lamda, iters):
    """
    梯度下降求使J(Ѳ)最小的Ѳ
    ...
    for it in range(0, iters):
        ...
        theta_4_new = theta_4 * (1 - alpha * lamda / len(X1)) - (alpha / len(X1)) * np.sum(np.multiply((theta_0 + theta_1 * X1 + theta_2 * X2 + theta_3 * X3 + theta_4 * X4 - y) , X4))
